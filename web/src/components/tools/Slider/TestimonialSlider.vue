    ...
    <div
      ...
      <div class="relative mb-6"
        ...
        <div class="pointer-events-none absolute left-1/2 top-0 -translate-x-1/2"
          ...
          <!-- 发光背景（可留可调） -->
          <div class="absolute inset-0 rounded-full bg-gradient-to-b from-purple-400/20 via-pink-300/10 to-transparent blur-xl"></div>

          <div class="flex h-full items-start justify-center [mask-image:_linear-gradient(0deg,transparent,theme(colors.white)_30%,theme(colors.white))]">
            <transition-group name="testimonial-image">
              <div v-for="(t, i) in testimonials" :key="`image-${i}`" v-show="active === i"
                   class="absolute left-1/2 -translate-x-1/2 avatar-pos">
                <div class="relative">
                  <div class="absolute inset-0 bg-gradient-to-r from-purple-400 to-pink-400 rounded-full blur-sm scale-110"></div>
                  <img :src="t.img" :alt="t.name"
                       class="relative rounded-full border-4 border-white/80 dark:border-zinc-800/80 shadow-2xl avatar-img" />
                </div>
              </div>
            </transition-group>
          </div>
      ...
      <div class="mb-8 transition-all duration-500 ease-out">
        <div ref="testimonialsRef" class="relative flex flex-col items-center">
      ...
      <div class="flex items-center justify-between pt-6 border-t border-zinc-200/50 dark:border-zinc-700/50">
        ...
        <button
            ...
            @click="handlePrev"
        >
          <i class="iconfont icon-shangyige text-zinc-600 dark:text-zinc-400 text-lg group-hover/button:-translate-x-0.5 transition-transform duration-300"></i>
        </button>
        ...
        <div class="flex flex-col items-center gap-3 flex-1 px-8">
          ...
          >
            <span class="relative z-10 flex items-center gap-2">
              <i class="iconfont icon-sparkles text-white text-lg group-hover/action:animate-pulse"></i>
              我想见她!
            </span>
            <!-- 按钮 hover 效果 -->
            <div
                class="absolute inset-0 bg-gradient-to-r from-white/20 to-transparent opacity-0 group-hover/action:opacity-100 transition-opacity duration-300"></div>
          </button>
        </div>
        ...
        <button
            ...
            @click="handleNext"
        >
          <i class="iconfont icon-next text-zinc-600 dark:text-zinc-400 text-lg group-hover/button:translate-x-0.5 transition-transform duration-300"></i>
        </button>
      </div>
...
.avatar-img { width: var(--av-size); height: var(--av-size); }
.avatar-pos { top: var(--av-gap); } /* 头像距区域顶部的“那一丢丢” */
</style>
